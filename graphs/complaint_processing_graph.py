...
from services.ai_services import detect_issue_from_image, map_issue_to_department
...
def vision_node(state: State) -> State:
    ...
    state["issue_type"] = detect_issue_from_image(state["image_url"])
    logger.info("Graph step=vision issue_type=%s", state.get("issue_type"))
    return state
...
builder.add_edge("vision", "department_mapping")
builder.add_edge("vision", "location")
builder.add_edge("department_mapping", "duplicate_check")
